    ...
    def update_weights(self, weights, td_errors_times_gradients):
        ...
        for i in range(len(weights)):
            for param in weights[i].keys():
                ...
                self.m[i][param] = self.m[i][param] * self.beta_m + (1 - self.beta_m) * td_errors_times_gradients[i][
                    param]
                self.v[i][param] = self.v[i][param] * self.beta_v + (1 - self.beta_v) * (
...
def get_td_error(states, next_states, actions, rewards, discount, terminals, network, current_q, tau):
    ...
    # Computing q_vec by selecting q(s, a) from q_mat for taken actions
    # Using batch_indices as the index for the first dimension of q_mat
...
def optimize_network(experiences, discount, optimizer, network, current_q, tau):
    ...
    # Computing TD error using the get_td_error function
    # q_vec is a 1D array of shape (batch_size)
    delta_vec = get_td_error(states, next_states, actions, rewards, discount, terminals, network, current_q, tau)

    batch_indices = np.arange(batch_size)

    # td error matrix of shape (batch_size, num_actions)
    ...
    # Passing the network.get_weights and the td_update to the optimizer to get updated weights
...
class Agent:
    ...
    def agent_step(self, reward, state):
        ...
        # state - an array of shape (1, state_dim) to match the get_action_values() and get_TD_update() functions
